# Governance — a commons with gardeners

This document is the constitution of Human Knowledge Roadmaps: who decides
what, and why it is built that way. The mission itself lives in the app's
About page; this is the machinery that protects it.
...
| **Gardener** (maintainer) | A named person per map | **Final say inside their map.** Reviews and merges proposals, keeps links alive, holds the map to the charter. |
| **Overseer** | A small editorial board | Decides **which maps exist**: intake, promotion, retirement. Appoints and replaces gardeners. Moderates any map and collection. |
| **Superadmin** | The project steward | Appoints and removes overseers and gardeners. Holds the infrastructure (repo, Firebase). Root role — granted only by hand in the Firebase console, never through the app or its API. |

Role bindings are public: they live in the world-readable `meta/roles`
document. Anyone can see who tends what.

## Who decides what

| Decision | Mechanism |
|---|---|
| Content and structure *within* a map | The map's gardener — one accountable voice. Coherence requires singular judgment. |
| Which maps exist; promotion to the canonical library | Overseers, deliberately conservative. The intake question is always: *why isn't this a topic in an existing map?* |
| Gardener appointment and succession | Overseers (and superadmin), based on demonstrated contribution. |
| Overseer appointment | Superadmin. |
| What's popular | **Nobody.** Usage signals (saves, paths started) are telemetry shown *to* gardeners and overseers as evidence — never a mechanism that directly decides anything. |
| Disagreement with a gardener's call | The fork valve (below), not a vote. |

## The editorial charter

Gardener authority is legitimate because it is bounded by public, checkable
rules. A gardener enforces the charter; they don't override it:
...
rejection that can't name its rule escalates to the overseers.

## How the library grows

**No gardener, no garden.** A map enters the canonical library only when a
...
scales with how many trustworthy stewards the community produces, while
every map keeps one accountable voice.

New maps start as proposals (an outline from the home screen), and an
overseer may land one as **incubating** with its proposer as founding
gardener. Promotion to the main library happens when it meets the charter,
...
1. **Collections** — anyone can group and sequence existing maps their own
   way and share it. "The gardener organized this wrong" has a productive
   answer that isn't a governance fight.
2. **Proposals** — every rejection is recorded; a pattern of good rejected
   ideas is evidence overseers can act on.
...
- A gardener holds a map in trust. Going dark for ~90 days with proposals
  waiting is a graceful handoff, not a scandal: overseers appoint a
  successor and the record thanks the departing gardener.
- Overseers and the superadmin can replace any role binding at any time;
...
merges them, after the overseers have seen them.
